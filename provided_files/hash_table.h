...
private:
  ...
  void resize(DynamicArray<T> &itemsArray, const int new_size);
...
bool HashTable<T>::insert(const T& item) {
  ...
  if (contains(item)) {
    ...
    if (numItems == tableSize){
      //resize table to double original size if it is saturated
      DynamicArray<T> itemsArray = getItemsArray();
      resize(itemsArray, tableSize*2);
    }
...
void HashTable<T>::remove(const T& item) {
  ...
  assert(link != NULL);
  if (numItems == tableSize/4 && tableSize > 10u){
      const int new_size = max(tableSize/2, 10u);
      //resize table to save memory if it is 3/4 empty
      DynamicArray<T> itemsArray = getItemsArray();
      resize(itemsArray, new_size);
      
      
  }
  bucket = getBucket(item);
  link = table[bucket].find(item);
...
template <typename T>
void HashTable<T>::resize(DynamicArray<T> &itemsArray, const int new_size) {
  /*
    Description:  replaces the old table with a new table of new size

    args: itemsArray = an array of elements to add to the new table
  ...
  */
  ...
  delete[] table;
  tableSize = new_size;
  //create a new table with one of updated size
  table = new LinkedList<T>[tableSize];
  for (int i = 0; i < numItems; i++){
    //put each item into the corresponding new bucket
    unsigned int buckets_buffer = getBucket(itemsArray[i]);
    table[buckets_buffer].insertFront(itemsArray[i]);
  }
}
